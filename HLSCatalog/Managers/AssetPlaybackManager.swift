...
class AssetPlaybackManager: NSObject {
    ...
    private var playerItem: AVPlayerItem? {
        willSet {
            ...
            guard let playerRateObserver = playerRateObserver else { return }
        ...
        didSet {
            ...
            playerRateObserver = player.observe(\.rate, changeHandler: self.onPlayerRateChange)
...
extension AssetPlaybackManager {
    private func onPlayerRateChange(player _: AVPlayer, change _: NSKeyValueObservedChange<Float>) {
        ...
        if player.rate > 0 {
            let queue = DispatchQueue(label: "com.hlsCatalog.fireTime.queue", qos: .background)
            let timeScale = CMTimeScale(NSEC_PER_SEC)
            let time = CMTime(seconds: self.fireTime, preferredTimescale: timeScale)
            playerPeriodicObservation = player.addPeriodicTimeObserver(forInterval:time, queue: queue)
            { [weak self] time in
                guard let strongSelf = self else { return }
                if (time.seconds > 0) {
                    strongSelf.delegate?.streamPlaybackManager(strongSelf, player: strongSelf.player, didPlayToTime: time)
                }
            }
        }
    }
...
protocol AssetPlaybackDelegate: class {
    ...
    /// This is called when the internal AVPlayer play to specific time (AssetPlaybackManager.fireTime)
    func streamPlaybackManager(_ streamPlaybackManager: AssetPlaybackManager, player: AVPlayer, didPlayToTime time: CMTime)
}
